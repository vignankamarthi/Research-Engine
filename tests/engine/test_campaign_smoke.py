...
def test_end_to_end_campaign_confirms(tmp_path):
    ...
    res = run_campaign(MockAgent(), MockBackend(0.25, 0.0, 0.1, seed=1), cfg(), ls, box_factory)
...
def test_catalog_drift_does_not_burn_a_box(tmp_path):
    ...
    with pytest.raises(ControlCatalogError):
        run_campaign(MockAgent(), MockBackend(0.25, 0.0, 0.1, seed=1), bad, ls, box_factory)
...
def test_triage_rejection_spends_no_box(tmp_path):
    ...
    res = run_campaign(MockAgent(), MockBackend(0.25, 0.0, 0.1, seed=1), cfg(), ls,
                       box_factory, triage=lambda narrative: False)
    assert res.verdict is None
    assert ls.box_status("box0") == "live"  # rejected at triage, no box touched


def test_geometry_artifact_campaign_fails_not_confirmed(tmp_path):
    ls = LeaseStore(str(tmp_path / "campaign.db"))
    ls.add_boxes(["box0"])
    # untrained reproduces the effect -> the FLOOR fails inside confirmation
    res = run_campaign(MockAgent(), MockBackend(0.25, 0.25, 0.1, seed=2), cfg(), ls, box_factory)
    assert res.verdict.status == "FAILED" and "floor" in res.verdict.reason
