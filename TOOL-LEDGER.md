...
| Entry | Purpose | Probe | Break signature | State | Status |
|---|---|---|---|---|---|
| Novelty verification CASCADE (Semantic Scholar -> arXiv -> OpenAlex) | novelty re-audit (PRE-allocation), Milestone 8 step 51. TIERED: S2 tier 1 (uses `S2_API_KEY` when set, higher rate limit), arXiv + OpenAlex keyless fallbacks. Each tier verifies the same way (query prior work, title-collision check), so a fallback is like-for-like | a freshness check (a known-recent item returns) per live tier | a source rate-limits (S2 429) or is unreachable -> tier down (explicit, logged, deterministic, NOT a silent fallback) | RETRY-then-CASCADE: try each source in order, first that answers wins; ALL tiers exhausted -> HARD STOP (`NoveltySourcesExhausted`, red flag, halt for human intervention), NEVER a silent fail-closed-reject of a scored finding. `cluster/campaign_real.py:_real_novelty_audit` | LIVE |
| Semantic Scholar MCP (generation) | grounded generation (scouts mine the derivative veins: limitations / future-work / contradictions), Milestone 8 step 51 | a freshness check (a known-recent item returns, corpus release id recorded) | stale cache returns old results | a stale corpus at GENERATION time only narrows breadth, never a validity risk | LIVE |
