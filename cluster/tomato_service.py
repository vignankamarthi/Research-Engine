...
requests over the shared file-RPC (same pattern as qwen_service.py, but MCQ instead of
...
Request JSON: {"items": [{"key","demonstration_type","question","options","answer"}, ...],
               "ablate_keep": null|float,        # the mechanism ablation (spectral mask)
               "untrained_seed": null|int}       # the FLOOR arm: a weights-randomized model at this seed
Response JSON: {"scores": [0.0, 1.0, ...]}       # per-item correctness (accuracy unit)

The untrained-init model is a genuinely weights-RANDOMIZED Qwen (`from_config`, not pretrained), so the
FLOOR gate measures a real geometry control rather than a stub. At most two untrained inits are cached
(pretrained + 2 randoms) to bound GPU memory. Stop by creating <rpc>/STOP. Run on a GPU node via sbatch."""
import json
import sys
import time
from pathlib import Path

TOMATO = Path("/work/neu/p2026_0016_neu/tomato")
...
def _untrained_model(seed, ssv2_qwen):
    """A weights-RANDOMIZED Qwen (from_config) at `seed`, cached (<=2) to bound GPU memory."""
    import torch
    from transformers import AutoConfig
    if seed in _UNTRAINED:
        return _UNTRAINED[seed]
    if len(_UNTRAINED) >= 2:
        old_seed, old = _UNTRAINED.popitem()
        del old
        torch.cuda.empty_cache()
    torch.manual_seed(int(seed))
    cfg = AutoConfig.from_pretrained(ssv2_qwen.MODEL_ID)
    # the constructor (not from_config, which this class lacks) builds a weights-RANDOMIZED model
